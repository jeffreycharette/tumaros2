...
if ($name=="edit") {
	...
	elseif ($_SESSION["user"]->username!="admin") {
		$_SESSION["history"]="";
		header('Location: http://demo.wearecharette.com/login.html');
	}
	/*else {
		$_GET['id']=2;
	}*/
...
if (file_exists($file)) {
	...
	if ($read_db->numRows($result) > 0) {
		...
		if ($mode!="edit") {
			$value['tpl']=str_replace("{facebook}",'<iframe src="http://www.facebook.com/plugins/like.php?href=http%253A%252F%252Fdemo.wearecharette.com%252F&amp;layout=standard&amp;show_faces=false&amp;width=470&amp;action=like&amp;font=arial&amp;colorscheme=light&amp;height=35" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:470px; height:35px;" allowTransparency="true"></iframe>',$value['tpl']);
		}
	}
	else {
	...
	if (is_array($query)) {
		foreach ($query as $key=>$v) {
			if (file_exists($key.".".$service)) {
				...
				if ($_GET['eid']!="" && is_numeric($_GET['eid'])) {
					$_GET['type']="override";
				}
				if (is_array($_GET)) {
				...
				$result=$read_db->query($sql);
				if ($read_db->numRows($result) > 0) {
					$editortpl_new="{name}{first}{last}{title}{type}{email}<div class=\"tdate\">{start}{end}{occurs}</div>{location}{instructor}{summary}{content}{info}{size}{price}{article}{news text}{news link}{text}{link name}{link}{text 1}{link 1}{text 2}{link 2}{text 3}{link 3}{text 4}{link 4}{signup link}{quote}{home caption}{caption}{images}";
					while ($row = $read_db->fetchArray()) {
						$model['type']=$row['type'];
						$model['name']=$row['name'];
						...
						elseif (stristr($model['type'],"image")) {
							...
							$imgs_new=print_input($model['name']."_".$model['type']."_".$next_id,$model['name'],"","","hidden_textarea"," id=\"image_new".$o."\" ")."<br /><label for=\"pictures\">IMAGE</label><ul class=\"imglist\" id=\"item_".$next_id."\"></ul><a id=\"images_".$next_id."_".$opts."\" class=\"upload\" href=\"#\" title=\"Click to upload\">upload</a>";
					...
					$newform="<div class=\"box\"><form name=\"entities\" method=\"post\" action=\"edit.html?id=".$cid."\"><h2 style=\"float:left;\">Add New</h2><input style=\"float:right\" value=\"Publish\" class=\"save\" type=\"submit\" /><hr /><input id=\"type\" type=\"hidden\" value=\"".$type."\" name=\"type\" /><input id=\"etpl\" type=\"hidden\" value=\"".$model['tpl']."\" name=\"etpl\" /><input id=\"create_check\" type=\"hidden\" value=\"1\" name=\"create_check\" />\n".$newform;
						...
						$lmt=" LIMIT ".$lmt;
                                        ...
                                        if ($options['field']!="" && $options['sort']!="") {
                                                if ($options['sort_type']=="numeric") {
                                                        $result=$read_db->query("SELECT collections.id FROM collections,cid_eid,entities WHERE ".$collection_type." cid_eid.cid=collections.i
     d AND cid_eid.eid=entities.id AND entities.name='".$options['field']."' AND collections.active='y' ORDER BY entities.value + 0 ".$options['sort']);
                                                }
                                                else {
...
." AND collections.active='y' GROUP BY collections.id".$lmt);
				...
				if (is_array($arr)) {
					...
					foreach ($arr as $k=>$v) {
						...
						$editortpl="{name}{first}{last}{title}{type}{email}<div class=\"tdate\">{start}{end}{occurs}{until}{dates}</div>{location}{instructor}{summary}{content}{info}{size}{price}{article}{news text}{news link}{text}{link name}{link}{text 1}{link 1}{text 2}{link 2}{text 3}{link 3}{text 4}{link 4}{signup link}{quote}{home caption}{caption}{images}";
						if ($options['id']!="") {
								...
								foreach ($v as $kk=>$vv) {
									if ($vv['type']!="tpl" && $kk!="tpl") {
										...
										elseif (stristr($vv['type'],"image")) {
											...
											$imgs="<br /><label for=\"pictures\">IMAGE</label><ul class=\"imglist\" id=\"item_".$kk."\"><li><a href=\"/uploaded/images/".$kk."/original.".$matches[1]."?".mt_rand()."\"><img src=\"/uploaded/images/".$kk."/100x100.".$matches[1]."?".mt_rand()."\" width=\"100\" height=\"100\" /></a></li></ul><a id=\"images_".$kk."_".$opts."\" class=\"upload\" href=\"#\" title=\"Click to upload\">upload</a>";
											$editortpl=str_replace("{".$vv['name']."}",$imgs,$editortpl);
											$arr[$k]['tpl']=str_replace("{small_image}","<img src=\"/uploaded/images/".$kk."/232x156.".$matches[1]."?".mt_rand()."\" width=\"232\" height=\"156\" />",$arr[$k]['tpl']);
										}
			...
			foreach ($value as $key=>$val) {
				...
				if ($key!="tpl" && $key!="id" && $val['type']!="query" && $val['type']!="tpl" && $val['type']!="javascript" && $val['type']!="meta") {
					...
					elseif (stristr($val['type'],"image")) {
						...
						$value['editor'].="<br /><label for=\"pictures\">IMAGE</label><ul class=\"imglist\" id=\"item_".$key."\"><li><a href=\"/uploaded/images/".$key."/original.".$matches[1]."?".mt_rand()."\"><img src=\"/uploaded/images/".$key."/100x100.".$matches[1]."?".mt_rand()."\" width=\"100\" height=\"100\" /></a></li></ul><a id=\"images_".$key."_".$opts."\" class=\"upload\" href=\"#\" title=\"Click to upload\">upload</a>";
					}
			...
			foreach ($value as $key=>$val) {
				if ($val['type']!="javascript") {
					...
					if ($key!="tpl" && $key!="id" && $val['type']!="query" && $val['type']!="tpl") {
						...
						elseif ($val['type']=="textarea" || $val['type']=="image") {
	...
	if ($mode!="edit") {
		$value['tpl']=str_replace("<!--google-->","<script type=\"text/javascript\">var _gaq = _gaq || []; _gaq.push(['_setAccount', 'UA-15944206-1']); _gaq.push(['_trackPageview']); (function() {var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true; ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js'; var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s); })(); </script>",$value['tpl']);
        }
...
function parseRequest($read_db) {
	...
	elseif(isset($_POST['emailform']) || isset($_POST['emailform_x']) || isset($_POST['emailform_y'])) {
...
function emailit() {
	$name=$_POST['name'];
	$content=email_action("jeffrey@wearecharette.com", "wearecharette Inquiry", "submit,submit_x,submit_y");
	if ($content=="true") {
        	return "Thank you ".$name." for submitting your email.";
        }
        else {
		return "Your email failed, check the address and try again.";
	}
}

function email_action($to="jeffrey@wearecharette.com", $subject="Information Request", $options="") {
$from=$_POST['email'];
if ($from=="") {
        $from=$to;
}
if (validateEmail($from)) {
        $html="<h2>New Request from " . ucwords($from) . "</h2><hr />";
        $plain="New Request from " . ucwords($from) . "
-------------------------------------";
        foreach ($_POST as $key => $value) {
        $key_name=str_replace("_"," ",$key);
                if ($key<>"submit" && $value<>"" && $key<>"submit_x" && $key<>"submit_y" && $key<>"emailform" && $key<>"emailform_x" && $key<>"emailform_y" && $key<>"cname" && $key<>"recaptcha_response_field" && $key<>"recaptcha_challenge_field" && $key<>"recaptcha_response_field") {
                        $html.="<b>" . $key_name . "</b> " . $value . "<br />";
                        $plain.="
" . $key . ": " . $value;
                }
                /*if (strlen($value) <= 0 && !stristr($options, $key)) {
                        $key_error=str_replace(" ","_",$key_name)."_e";
                        //$errors[$key_error]="Please fill in the " . ucwords($key_name) ." field.";
                        $errors[$key_error]="fill this out";
                }*/
        }
        if ($_POST["recaptcha_response_field"]) {
                        require_once('/_private/recaptcha-1.1/recaptchalib.php');
                        ...
                        if ($resp->is_valid) {
                                        $valid_recaptcha=$resp->is_valid;
                        } else {
                                        # set the error code so that we can display it
                                        if ($resp->error=="incorrect-captcha-sol") {
                                                $errors['recaptcha_e'] = "Invalid Captcha, please try again";
                                        }
                                        else {
                                                $errors['recaptcha_e'] = $resp->error;
                                        }
                        }
        }
        if (is_array($errors)) {
...
//end heredoc
        ...
        /*require('/_private/Swift.php');
        require('/_private/Swift/Swift_SMTP_Connection.php');
        $connection = new Swift_SMTP_Connection('k2smtpout.secureserver.net');
        //require('/_private/Swift/Swift_Sendmail_Connection.php');
        //$connection = new Swift_Sendmail_Connection('/usr/sbin/sendmail');
        ...
        if (!$mailer->hasFailed()) {
                ...
                $errors['message_e'] = "<h2>request failed</h2><p>Please try again later or use our other contact info such as our business email.</p>";
        }*/
        ...
        // Additional headers
        //$headers .= 'To: HATCHWARE.com <'.$to.'>' . "\r\n";
        //$headers .= 'From: '.$from."\r\n";
        //mail("", $subject, $html, $headers);
        sendElasticEmail($from, $to, "SJICA CMS", "SJICA Web Request", $html, "true");
	return true;
}
else {
...
function sendElasticEmail($from, $to, $fromName, $subject, $body, $isHtml) {
    $data = "username=iomatix@yahoo.com";
    $data .= "&api_key=19974257-c44b-4b40-90df-a88cfd7961b6";
    $data .= "&from=".$from;
    ...
    $fp = @fopen('https://elasticemail.com/_api/mailer/send', 'rb', false, $ctx);
							...
							if (exif_imagetype($tmp_name)) {
								...
								if (is_array($returnval)) {
									...
									if (is_array($returnval)) {
										...
										$returnval=resize($tmp_name,232,156,$dir_id."/232x156.".$extension,FALSE,TRUE);
										if (is_array($returnval)) {
											$error=$returnval['message_e'];
										}
									}
	...
	if(isset($_POST["submit"]) || isset($_POST["submit_x"]) || isset($_POST["submit_y"])) {
		...
		if (!is_array($errors)) {
			...
			if($read_db->numRows($result) == 1) {
				...
				header('Location: http://demo.wearecharette.com/edit.html');
			}
			else {
...
function resize($img, $thumb_width, $thumb_height, $newfilename, $square=FALSE, $crop=FALSE) {
	...
	if ($crop) {
		...
		switch ($image_type) {
			...
			case 2: imagejpeg($thumb,$newfilename);  break;
